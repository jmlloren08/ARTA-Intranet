...
const DropdownNotification = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      setNotifying(data.length > 0 ? true : false);
    } catch (error) {
  ...
  const markNotificationAsRead = async (id) => {
    try {
      const response = await axios.post(`/notifications/${id}/mark-as-read`);
  ...
  return (
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      <li>
        ...
        >
          {notifying && <span className='absolute -top-0.5 right-0 z-1 h-2 w-2 rounded-full bg-meta-1'>
          ...
          >
            ...
            <ul className="flex h-auto flex-col overflow-y-auto">
              {notifications.map((notification, index) => (
                <li key={notification.id} className='border-t border-stroke'>
                  <Link
                    onClick={() => {
                      markNotificationAsRead(notification.id);
                    }}
                    href={route('activity-tracker-list')}
